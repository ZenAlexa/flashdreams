...
# source makes the candidate side (``flashvsr.impl.transformer``,
# ``flashvsr.impl.decoder.network``) importable here, replacing the previous
...
# through ``flashvsr.impl.transformer.network`` instead.
if ! ( cd "${SCRIPT_DIR}" && \
...
#     ``flashvsr.impl.decoder.network.FlashVSR_TAEHV``.
...
#     and asserts parity against the live ``flashvsr.impl.transformer.FlashVSRTransformer``.
